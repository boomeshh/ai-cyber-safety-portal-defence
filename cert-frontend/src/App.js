...
export default function App() {
  ...
  const loadData = async () => {
    try {
      const res = await fetch(`${API}/cert/intel`, {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const fileName = decodeURIComponent(fileNameMatch?.[1] || fileNameMatch?.[2] || 'complaints_export.xlsx');
      ...
      alert(`${err.message}\n\nIf backend shows Excel error, run: pip install openpyxl`);
    } finally {
      setExporting(false);
    }
  };
  ...
  if (!user || !['cert', 'admin'].includes(user.role)) {
      ...
      <div className="cert-shell">
        <div className="hero">
          <div>
            ...
            <p>
              Protected operational dashboard for live alerts, escalations,
              linked campaigns, evidence-ready review, and OPSEC-sensitive
              incidents.
            </p>
          </div>
        ...
        {summary?.critical_alerts > 0 ? (
          <div className="alert-banner">
            🚨 ACTIVE CRITICAL ALERTS: {summary.critical_alerts} case(s) require
            immediate CERT review.
          </div>
        ) : (
          ...
          <div className="panel">
            ...
            {graph?.nodes?.length ? (
              ...
              <div className="empty-state">
                No linked campaign graph available yet.
              </div>
            )}
          </div>
          ...
          <div className="panel">
            ...
            <div className="list-wrap">
              {(feed?.opsec_cases || []).length === 0 ? (
                ...
                feed.opsec_cases.map((item) => (
                  <IncidentCard key={item.id} item={item} showReason />
                ))
              )}
            </div>
          </div>
          ...
          <div className="panel">
            ...
            <div className="list-wrap">
              {(feed?.repeated_cases || []).length === 0 ? (
                ...
                feed.repeated_cases.map((item) => (
                  <IncidentCard key={item.id} item={item} campaign />
                ))
              )}
            </div>
          </div>
...
function SummaryCard({ label, value, critical }) {
  return (
    <div className={`summary-card ${critical ? 'critical-card' : ''}`}>
      ...
      <div className="summary-value">{value ?? 0}</div>
    </div>
  );
}
        ...
        rows.map(([label, value]) => (
          <div className="bar-row" key={label}>
            ...
            <div className="bar-track">
              <div className="bar-fill" style={{ width: `${(Number(value || 0) / max) * 100}%` }}></div>
            </div>
    ...
    <div className="graph-grid">
      ...
      <div className="graph-edge-list">
        ...
        {graph.edges.map((edge, index) => (
          <div key={`${edge.source}-${edge.target}-${index}`} className="edge-item">
